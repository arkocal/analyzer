...
module ConstrSys = struct
  ...
  (*
    ...
    *)
  let system : LVar.t -> ((LVar.t -> D.t) -> (LVar.t -> D.t -> unit) -> (LVar.t -> unit) option -> (GVar.t -> G.t) -> (GVar.t -> G.t -> unit) -> D.t) option = function
...
module Solver = GlobSolverFromEqSolver (PostSolver.CreatingEqIncrSolverFromEqIncrSolver(PostSolver.EqIncrSolverFromEqSolver (EffectWConEq.Make)) (PostSolverArg)) (ConstrSys) (LH) (GH)
